...
import {
  ...
  XCircle, Clock, RefreshCw, Zap, Globe, Target,
...
/* ═══════════════════════════════════════════════════════════════════════════════
   ...
   Institutional executive intelligence workspace.
   Analytics-driven, export-ready, calm insight stream.
   ═══════════════════════════════════════════════════════════════════════════════ */
...
// ── ANALYTICS SURFACE ─────────────────────────────────────────────────────────
// ══════════════════════════════════════════════════════════════════════════════
function AnalyticsSurface({ branchFilter, months }: { branchFilter: string; months: number }) {
                    ...
                    return (
                      <div key={b.branchId} className="space-y-1">
                        ...
                        <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                          <div className="h-full rounded-full transition-all duration-700" style={{ width: `${pct}%`, background: P.series[i % P.series.length] }} />
                        </div>
                      </div>
                    );
                  })}
                  ...
                  <tbody>
                    {(branches ?? []).map((b, i) => (
                      <tr key={b.branchId} className="border-b last:border-0 hover:bg-xedu-slate-50/80 dark:hover:bg-xedu-slate-700/30 transition-colors">
                        ...
                        <td className="py-2.5 text-center">
                          <Badge variant={b.conversionRate >= 20 ? 'default' : b.conversionRate >= 10 ? 'secondary' : 'outline'} className="text-xs">
                            {b.conversionRate}%
                          </Badge>
                        </td>
                      </tr>
                  ...
                  {(marketing?.funnelBySource ?? []).map((r, i) => {
                    ...
                    return (
                      <div key={r.source} className="space-y-1">
                        ...
                        <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                          <div className="h-full rounded-full" style={{ width: `${pct}%`, background: P.series[i % P.series.length] }} />
                        </div>
                      </div>
                    );
                  })}
        ...
        {alertLoading ? (
          ...
          <div className="space-y-2">
            {(alerts ?? []).map((alert, i) => {
              ...
              return (
                <div key={i} className={`flex items-start gap-3 rounded-xl border px-4 py-3 ${cfg.bg} ${cfg.border}`}>
...
// ── ATTENDANCE SURFACE ────────────────────────────────────────────────────────
// ══════════════════════════════════════════════════════════════════════════════
function AttendanceSurface({ dateRange, dateKey }: { dateRange: { from: string; to: string }; dateKey: number }) {
...
// ── GRADES SURFACE ────────────────────────────────────────────────────────────
// ══════════════════════════════════════════════════════════════════════════════
function GradesSurface({ dateRange, dateKey }: { dateRange: { from: string; to: string }; dateKey: number }) {
  const { data: report, isLoading } = useQuery({
...
// ── FINANCE SURFACE ───────────────────────────────────────────────────────────
// ══════════════════════════════════════════════════════════════════════════════
function FinanceSurface({ dateRange, dateKey }: { dateRange: { from: string; to: string }; dateKey: number }) {
  const { data: report, isLoading } = useQuery({
...
// ══════════════════════════════════════════════════════════════════════════════
function IntelligenceSidebar({ activeTab }: { activeTab: string }) {
  const { data: pulse, isLoading: pulseLoading } = useQuery<SchoolPulse>({
    queryKey: ['analytics', 'pulse'],
    queryFn: () => analyticsApi.getPulse(),
    refetchInterval: 60_000,
    ...
    <>
      ...
      <WorkspaceSection title="Pulse" icon={<Zap className="h-4 w-4 text-xedu-amber-500" />}>
        {pulseLoading ? (
          ...
          <div className="grid grid-cols-2 gap-2">
            <StatPill label="O'quvchilar" value={pulse?.totalStudents ?? 0} />
            <StatPill label="Davomat" value={pulse?.today.attendanceRate != null ? `${pulse.today.attendanceRate}%` : '—'} />
            <StatPill label="Kirim" value={fmt(pulse?.monthlyRevenue ?? 0)} tone="success" />
            <StatPill label="Qarz" value={fmt(pulse?.pendingDebt.amount ?? 0)} tone={pulse?.pendingDebt.count ? 'attention' : 'calm'} />
          </div>
        )}
      </WorkspaceSection>
      ...
      >
        {alertLoading ? (
          ...
          <div className="flex items-center gap-2 text-xedu-slate-500 py-2">
            <CheckCircle className="h-4 w-4 text-xedu-primary" />
            <span className="text-xs">Hamma narsa yaxshi</span>
          </div>
        ) : (
      ...
      {/* Tab-specific tip */}
      {activeTab === 'analytics' && (
        <WorkspaceSection title="Maslahat" icon={<Globe className="h-4 w-4 text-xedu-primary" />}>
          <p className="text-xs text-xedu-slate-500 leading-relaxed">
            Analytics dashboard real-vaqt ma'lumotlarini ko'rsatadi. Branch filter orqali ma'lum filialni tahlil qiling.
          </p>
        </WorkspaceSection>
      )}
      {activeTab === 'attendance' && (
        <WorkspaceSection title="Maslahat" icon={<CheckCircle className="h-4 w-4 text-xedu-primary" />}>
          <p className="text-xs text-xedu-slate-500 leading-relaxed">
            Davomat hisobotini sana oraliqini tanlab chop eting. PDF yuklab olish tugmasi orqali bosma shaklda saqlang.
          </p>
        </WorkspaceSection>
      )}
      {activeTab === 'grades' && (
        <WorkspaceSection title="Maslahat" icon={<TrendingUp className="h-4 w-4 text-xedu-primary" />}>
          <p className="text-xs text-xedu-slate-500 leading-relaxed">
            Baho taqsimoti va fan bo'yicha o'rtacha ballarni kuzating. Past ballar e'tibor talab qiladi.
          </p>
        </WorkspaceSection>
      )}
      {activeTab === 'finance' && (
        <WorkspaceSection title="Maslahat" icon={<BarChart3 className="h-4 w-4 text-xedu-primary" />}>
          <p className="text-xs text-xedu-slate-500 leading-relaxed">
            Moliyaviy hisobot oylik dinamika va qarzdorlar ro'yxatini aks ettiradi. Kechikkan to'lovlarga e'tibor bering.
          </p>
        </WorkspaceSection>
      )}

      {/* Quick links */}
      <WorkspaceSection title="Tezkor havolalar" icon={<ChevronRight className="h-4 w-4 text-xedu-slate-400" />}>
        <div className="space-y-1">
          <a href="/dashboard/payments" className="flex items-center gap-2 rounded-md px-2 py-1.5 text-xs font-medium text-xedu-slate-600 hover:bg-xedu-slate-50 dark:hover:bg-xedu-slate-800 transition-colors">
            <TrendingUp className="h-3.5 w-3.5 text-xedu-slate-400" /> To'lovlar
          </a>
          <a href="/dashboard/attendance" className="flex items-center gap-2 rounded-md px-2 py-1.5 text-xs font-medium text-xedu-slate-600 hover:bg-xedu-slate-50 dark:hover:bg-xedu-slate-800 transition-colors">
            <CheckCircle className="h-3.5 w-3.5 text-xedu-slate-400" /> Davomat
          </a>
          <a href="/dashboard/students" className="flex items-center gap-2 rounded-md px-2 py-1.5 text-xs font-medium text-xedu-slate-600 hover:bg-xedu-slate-50 dark:hover:bg-xedu-slate-800 transition-colors">
            <Users className="h-3.5 w-3.5 text-xedu-slate-400" /> O'quvchilar
          </a>
          <a href="/dashboard/schedule" className="flex items-center gap-2 rounded-md px-2 py-1.5 text-xs font-medium text-xedu-slate-600 hover:bg-xedu-slate-50 dark:hover:bg-xedu-slate-800 transition-colors">
            <Clock className="h-3.5 w-3.5 text-xedu-slate-400" /> Jadval
          </a>
        </div>
      </WorkspaceSection>
    </>
...
export function ReportsWorkspace() {
  ...
  const handleExportExcel = async (type: 'students' | 'payments' | 'attendance') => {
    setExporting(type);
    try {
      await analyticsApi.downloadExcel(type, branchFilter || undefined);
      toast({ title: 'Excel tayyor!', description: `${type} fayli yuklab olindi.` });
    ...
    <WorkspaceShell layout="two-column" density="compact">
      ...
      <WorkspaceMain>
        <div ref={activeTab !== 'analytics' ? printRef : undefined}>
          {activeTab === 'analytics'  && canSeeAnalytics && <AnalyticsSurface branchFilter={branchFilter} months={months} />}
          {activeTab === 'attendance' && <AttendanceSurface dateRange={dateRange} dateKey={dateKey} />}
          {activeTab === 'grades'     && <GradesSurface     dateRange={dateRange} dateKey={dateKey} />}
          {activeTab === 'finance'    && canSeeFinance     && <FinanceSurface dateRange={dateRange} dateKey={dateKey} />}
        </div>
      </WorkspaceMain>

      {/* Intelligence sidebar */}
      <WorkspaceSidebar width="narrow">
        <IntelligenceSidebar activeTab={activeTab} />
      </WorkspaceSidebar>
    </WorkspaceShell>
  );
}
